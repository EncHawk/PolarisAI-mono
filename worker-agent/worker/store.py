"""Shared Upstash Redis (REST/HTTPS) + supabase singletons for the worker.

The worker used to talk to a local redis-server via redis-py; it now uses the
...
`hset(..., mapping=...)`, `blpop(key, timeout=0)` and a no-op `pubsub()`).
"""
