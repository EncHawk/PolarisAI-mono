...
type CheckoutResponse = { checkout_url: string; session_id: string; plan: Plan }
...
function Workspace({ user, onLogout, onBack }: { user: User; onLogout: () => void; onBack: () => void }) {
  ...
  const refresh = async () => { if (!result) return; setBusy(true); try { const checkout = await api<CheckoutResponse>('/billing/checkout', { method: 'POST', body: JSON.stringify({ plan: 'starter', job_uuid: result.job_uuid }) }); window.location.assign(checkout.checkout_url) } catch (err) { setError(err instanceof Error ? err.message : 'Could not open Stripe checkout') } finally { setBusy(false) } }
